...
app.post('/api/seed/admin', async (req, res) => {
  try {
    ...
    // Create master admin - password will be hashed by pre-save hook
    const admin = await Admin.create({
      name: 'Master Admin',
      email: 'admin@dealcross.net',
      password: 'Admin123!@#Strong', // This will be hashed automatically
      role: 'master',
      status: 'active',
      permissions: {
        viewTransactions: true,
...
const server = app.listen(PORT, () => {
  ...
  console.log(`   - GET  /api/test-emergency`);
  console.log(`   - GET  /api/emergency/db-info`);
  console.log(`   - GET  /api/emergency/list-admins`);
  console.log(`   - GET  /api/emergency/check-admin/:email`);
