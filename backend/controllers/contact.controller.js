...
// Uses Resend for email (add RESEND_API_KEY to Render env vars)
...
exports.submitContact = async (req, res) => {
  try {
    ...
    await resend.emails.send({
      from: FROM_EMAIL, to: [SUPPORT_EMAIL], replyTo: email,
      subject: `[Contact Form] ${subject}`,
      html: `<div style="font-family:Arial;max-width:600px">
        <div style="background:#1d4ed8;color:#fff;padding:20px;border-radius:8px 8px 0 0"><h2 style="margin:0">New Contact Form</h2></div>
        <div style="background:#f9fafb;padding:24px;border-radius:0 0 8px 8px">
          <p><strong>Name:</strong> ${name}</p>
          <p><strong>Email:</strong> <a href="mailto:${email}">${email}</a></p>
          <p><strong>Subject:</strong> ${subject}</p>
          <p><strong>Message:</strong></p>
          <div style="background:#fff;padding:12px;border-radius:6px;border:1px solid #e5e7eb">${message.replace(/\n/g,'<br>')}</div>
        </div>
        <p style="color:#9ca3af;font-size:11px">Via dealcross.net · Aba, Abia State, Nigeria</p>
      </div>`
...
exports.subscribeNewsletter = async (req, res) => {
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      // Notify admin
      await resend.emails.send({
        from: FROM_EMAIL, to: [SUPPORT_EMAIL],
