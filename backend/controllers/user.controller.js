...
/**
 * ✅ DEBUG: Check KYC status from both DB and DIDIT API
 */
exports.debugKYCStatus = async (req, res) => {
  try {
    ...
    // Try to get status from Didit if we have a session ID
    if (user.kycStatus?.diditSessionId) {
      try {
        console.log('🔍 Checking Didit verification status:', user.kycStatus.diditSessionId);
        const response = await diditService.getVerificationStatus(
          user.kycStatus.diditSessionId
        );
        
        if (response.success) {
          diditStatus = response.data;
        } else {
          diditError = response.error || response.message;
        }
      } catch (error) {
    ...
    res.json({
      ...
      data: {
        ...
        // KYC status from Didit API
        diditAPI: diditStatus ? {
          status: diditStatus.status,
          verified: diditStatus.verified,
          failureReason: diditStatus.failureReason
        } : null,
