...
const escrowSchema = new mongoose.Schema({
  ...
  transactionType: { type: String, enum: Object.values(ESCROW_TYPES), default: 'custom' },
  category: { type: String, enum: ['goods', 'services', 'digital', 'real_estate', 'vehicle', 'other'], default: 'other' },
  ...
  participants: [{
    ...
    role: { type: String, enum: Object.values(PARTICIPANT_ROLES), required: true },
  ...
  amount: { type: mongoose.Schema.Types.Decimal128, required: [true, 'Amount is required'], min: 0 },
  currency: { type: String, required: true, enum: [...SUPPORTED_CURRENCIES.FIAT, ...SUPPORTED_CURRENCIES.CRYPTO], default: 'USD' },
  currencyType: { type: String, enum: ['fiat', 'crypto'], default: 'fiat' },
  ...
  payment: {
    ...
    // ✅ FIX: Fields were missing from schema — Mongoose silently drops unknown
    // fields on save(), so payment.reference was never persisted to MongoDB.
    // Every verify call returned "Escrow not found for this payment reference".
    reference: String,       // gateway reference: PAY_ESC{id}_{timestamp}
    method: String,          // 'paystack' | 'flutterwave' | 'crypto'
    paymentId: String,       // NowPayments crypto payment ID
    verifiedAt: Date,        // timestamp when verification completed
    transactionId: String,   // gateway transaction ID
    gatewayResponse: mongoose.Schema.Types.Mixed,  // raw gateway response object
  ...
  milestones: [{
    ...
    deliverables: [String],
    dueDate: Date,
    status: { type: String, enum: ['pending', 'in_progress', 'submitted', 'approved', 'rejected', 'paid'], default: 'pending' },
    startedAt: Date,
    submittedAt: Date,
    approvedAt: Date,
    rejectedAt: Date,
  ...
  status: { type: String, enum: Object.values(ESCROW_STATUS), default: 'pending' },
  timeline: [{
    status: String,
  ...
  delivery: {
    method: { type: String, enum: ['physical', 'digital', 'service', 'in_person', 'crypto'], default: 'physical' },
    status: { type: String, enum: ['pending', 'in_transit', 'delivered', 'confirmed', 'rejected'], default: 'pending' },
    deliveredAt: Date,
    confirmedAt: Date,
    trackingNumber: String,
    courierService: String,
    shippingAddress: { street: String, city: String, state: String, country: String, zipCode: String },
    proofOfDelivery: [{ type: String, url: String, uploadedAt: Date }],
  ...
  inspection: {
    ...
    scheduledDate: Date,
    completedDate: Date,
    status: { type: String, enum: ['not_required', 'scheduled', 'in_progress', 'passed', 'failed'], default: 'not_required' },
    fee: { type: Number, default: 0 },
    report: { summary: String, details: String, photos: [String], passed: Boolean, issues: [String], recommendations: [String] }
  },
  ...
  dispute: {
    ...
    description: String,
    evidence: [String],
    status: { type: String, enum: ['none', 'pending', 'under_review', 'resolved', 'closed'], default: 'none' },
    arbitrator: { type: mongoose.Schema.Types.ObjectId, ref: 'User' },
    assignedAt: Date,
    resolution: String,
    resolutionType: { type: String, enum: ['refund_buyer', 'release_to_seller', 'partial_refund', 'split_payment'] },
  ...
  attachments: [{
    ...
    mimetype: String,
    size: Number,
    category: { type: String, enum: ['contract', 'invoice', 'receipt', 'proof', 'other'], default: 'other' },
    uploadedBy: { type: mongoose.Schema.Types.ObjectId, ref: 'User' },
...
escrowSchema.methods.getUserRoles = function(userId) {
  return this.participants.filter(p => p.user.toString() === userId.toString()).map(p => p.role);
};

escrowSchema.methods.canUserAccess = function(userId) {
  if (this.buyer.toString() === userId.toString() || this.seller.toString() === userId.toString()) return true;
...
escrowSchema.methods.updateStatus = function(newStatus, actorId, actorRole = '', note = '') {
  ...
  this.timeline.push({ status: newStatus, actor: actorId, actorRole, note, timestamp: new Date() });
  return this.save();
};
...
escrowSchema.methods.getRemainingMilestoneAmount = function() {
  const paidAmount = this.milestones.filter(m => m.status === 'paid').reduce((sum, m) => sum + parseFloat(m.amount.toString()), 0);
...
escrowSchema.methods.addChatMessage = function(senderId, senderRole, message, attachments = []) {
  this.chat.push({ sender: senderId, senderRole, message, timestamp: new Date(), read: false, attachments });
  return this.save();
};
...
escrowSchema.methods.markMessagesAsRead = function(userId) {
  this.chat.forEach(msg => {
    if (msg.sender.toString() !== userId.toString()) {
      ...
      if (!alreadyRead) msg.readBy.push({ user: userId, readAt: new Date() });
    }
  });
  return this.save();
};
...
escrowSchema.methods.getUnreadCount = function(userId) {
  return this.chat.filter(msg => msg.sender.toString() !== userId.toString() && !msg.read).length;
};
...
escrowSchema.methods.raiseDispute = function(userId, userRole, reason, description, evidence = []) {
  ...
  this.dispute = { isDisputed: true, raisedBy: userId, raisedByRole: userRole, raisedAt: new Date(), reason, description, evidence, status: 'pending' };
  return this.save();
};
...
escrowSchema.methods.resolveDispute = function(resolvedBy, resolution, resolutionType, amounts = {}) {
  ...
  else if (['release_to_seller', 'partial_refund', 'split_payment'].includes(resolutionType)) this.status = 'completed';
  return this.save();
};
...
escrowSchema.methods.calculateTotalFees = function() {
  ...
  return ['buyerFee', 'sellerFee', 'agentFee', 'arbitratorFee', 'inspectorFee']
    .reduce((sum, f) => sum + (p[f] ? parseFloat(p[f].toString()) : 0), 0);
};
...
escrowSchema.methods.shouldAutoRelease = function() {
  if (!this.delivery.autoReleaseEnabled || !this.delivery.autoReleaseAt) return false;
...
escrowSchema.methods.requiresUserAction = function(userId) {
  ...
  if (userRoles.includes('inspector') && this.status === 'awaiting_inspection' && this.inspection.status === 'in_progress') return 'complete_inspection';
  if (userRoles.includes('arbitrator') && this.dispute.isDisputed && this.dispute.status === 'under_review') return 'resolve_dispute';
  return null;
};
...
escrowSchema.methods.addParticipant = function(userId, role, compensation = 0, compensationPercentage = 0) {
  ...
  this.participants.push({ user: userId, role, status: 'invited', invitedAt: new Date(), compensation, compensationPercentage });
  return this.save();
};
