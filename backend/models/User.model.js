// backend/models/User.model.js - FIXED
...
const userSchema = new mongoose.Schema({
  ...
  kycStatus: {
    status: {
      type: String,
      enum: [
        'unverified', 'pending', 'in_progress', 'under_review',
        'approved', 'rejected', 'expired', 'pending_documents',
      ],
      default: 'unverified',
    },
    submittedAt: Date,
    ...
    verificationMethod: { type: String, enum: ['didit', 'manual'], default: 'didit' },
    accountType: String,
    documents: [{
      type: { type: String },
      url: String,
  ...
  businessInfo: {
    ...
    industry: {
      ...
      enum: [
        ...
        'manufacturing', 'retail', 'technology', 'fashion', 'automotive',
        'services', 'other',
      ],
    },
    businessAddress: {
      ...
      city: String,
      state: String,
      country: String, // Free-text — no enum restriction
      zipCode: String,
    },
  ...
  address: {
    ...
    city:    String,
    state:   String,
    // Free-text country — the old enum was blocking saves for unlisted countries
    country: { type: String, trim: true },
    zipCode:   String,
    postalCode: String,
    formatted: String,
...
userSchema.pre('save', function (next) {
  // Sync KYC verification flag
  this.isKYCVerified = this.kycStatus?.status === 'approved';

  // Sync top-level country from address if not set
  ...
  if (this.createdAt) {
    this.stats.accountAgeDays = Math.floor((now - this.createdAt) / (1000 * 60 * 60 * 24));
  }
...
userSchema.methods.canAccessEscrow = function () {
  ...
  if (this.status !== 'active') {
    return { allowed: false, reason: 'Account not active', accountStatus: this.status };
  }
...
userSchema.methods.requiresManualKYC = function () {
  return (
    this.getVerificationMethod() === 'manual' &&
...
userSchema.methods.getRequiredDocuments = function () {
  ...
  const documentTemplates = {
    Nigeria:          { registration: 'CAC Certificate of Incorporation', tax: 'Tax Identification Number (TIN)', idType: 'NIN, Driver\'s License, or International Passport' },
    Kenya:            { registration: 'Certificate of Incorporation', tax: 'KRA PIN Certificate', idType: 'National ID or Passport' },
    Ghana:            { registration: 'Company Registration Certificate', tax: 'TIN Certificate', idType: 'Ghana Card or Passport' },
    'South Africa':   { registration: 'CIPC Registration Certificate', tax: 'Tax Clearance Certificate', idType: 'ID Book or Passport' },
    'United States':  { registration: 'Articles of Incorporation', tax: 'EIN Confirmation Letter', idType: 'Driver\'s License or Passport' },
    USA:              { registration: 'Articles of Incorporation', tax: 'EIN Confirmation Letter', idType: 'Driver\'s License or Passport' },
    'United Kingdom': { registration: 'Certificate of Incorporation', tax: 'UTR (Unique Taxpayer Reference)', idType: 'Passport or Driver\'s Licence' },
    UK:               { registration: 'Certificate of Incorporation', tax: 'UTR (Unique Taxpayer Reference)', idType: 'Passport or Driver\'s Licence' },
  };
...
// Single export — the duplicate was causing a crash on startup
