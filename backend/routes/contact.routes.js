...
contactRouter.post('/', async (req, res) => {
  try {
    ...
    // Save to database first (always - regardless of email)
    try {
      await ContactSubmission.create({ name, email, subject, message });
    } catch (dbErr) {
      console.error('ContactSubmission save error:', dbErr.message);
    }

    // Send emails via Resend (if key configured)
    if (process.env.RESEND_API_KEY) {
      // Notify admin - this always works (same domain)
      ...
      // Auto-reply to sender - non-fatal if it bounces
      resend.emails.send({
        from: FROM_EMAIL,
        to:   [email],
      ...
      }).catch(e => console.error('Auto-reply error (non-fatal):', e.message));
    }
  ...
  try {
    ...
    try {
      ...
      if (dbErr.code === 11000) {
        ...
        console.error('NewsletterSubscriber save error:', dbErr.message);
      }
    }
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      // Confirmation to subscriber - non-fatal if it bounces
      resend.emails.send({
        from: FROM_EMAIL,
        to:   [email],
      ...
      }).catch(e => console.error('Newsletter confirm error (non-fatal):', e.message));
    }
