...
const ProfilePage = () => {
  ...
  // Use refs to track mounted state and prevent multiple calls
    ...
    try {
      ...
      if (isMounted.current) {
        ...
        if (kycResponse.status === 'fulfilled' && kycResponse.value.success) {
          ...
          // Set default KYC status on error
  ...
  const handleTabChange = (tab) => {
    // Don't set searchParams to avoid re-renders, just update state
    setActiveTab(tab);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      {/* Header - Simplified to prevent re-renders */}
...
const KYCVerificationWarning = ({ kycStatus, onVerifyClick }) => {
  ...
  if (kycStatus?.status === 'pending') {
    ...
    buttonText = null; // No button for pending
