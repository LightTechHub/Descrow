// src/pages/Profile/ProfileTab.jsx - FIXED
...
const ProfileTab = ({ user, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Build payload — strip empty enum fields so Mongoose doesn't get ''
      const payload = {
        name:        formData.name,
      ...
      if (user.accountType === 'business') {
        ...
        // Remove empty strings for enum-typed fields only
      ...
      if (response.success) {
        ...
        setEditMode(false);
        // Re-fetch profile so parent state (and the header) reflects new data
        onUpdate && onUpdate();
      }
    } catch (error) {
  ...
  // The name shown at the top — company name for business, personal name for individual
  ...
  const getAvatarInitial = () => {
    if (user.accountType === 'business') {
      return user.businessInfo?.companyName?.charAt(0)?.toUpperCase() ||
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
          ...
          <div className="flex items-center gap-6 pb-6 border-b border-gray-200 dark:border-gray-800">
            ...
            <div className="relative">
              ...
              <div className={`w-24 h-24 rounded-full bg-blue-600 flex items-center justify-center text-white text-3xl font-bold border-4 border-gray-200 dark:border-gray-700 fallback-avatar ${user.profilePicture ? 'hidden' : ''}`}>
                {getAvatarInitial()}
              </div>
              {uploading && (
            ...
            <div className="flex-1">
              {/* Company name (business) or personal name */}
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{getDisplayName()}</h2>
              <p className="text-gray-600 dark:text-gray-400 mb-3">{user.email}</p>

              {/* Show owner name beneath company name for business accounts */}
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <InfoItem icon={Calendar} label="Member Since"  value={formatMemberSince()} />
          </div>
          {user.bio && (
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                {[user.address?.city, user.address?.state, user.address?.postalCode || user.address?.zipCode, user.address?.country || user.country]
                  .filter(Boolean).join(', ')}
              </p>
            </div>
          </div>
        ...
        {user.accountType === 'business' && user.businessInfo && (
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {user.businessInfo?.companyName        && <InfoItem icon={Building2} label="Company Name"         value={user.businessInfo.companyName} />}
              {user.businessInfo?.companyType        && <InfoItem icon={Briefcase} label="Company Type"         value={user.businessInfo.companyType.replace(/_/g, ' ').toUpperCase()} />}
              {user.businessInfo?.industry           && <InfoItem icon={Briefcase} label="Industry"             value={user.businessInfo.industry.replace(/_/g, ' ')} />}
              {user.businessInfo?.taxId              && <InfoItem icon={Hash}      label="Tax ID"               value={user.businessInfo.taxId} />}
              {user.businessInfo?.registrationNumber && <InfoItem icon={Hash}      label="Registration Number"  value={user.businessInfo.registrationNumber} />}
              {user.businessInfo?.website            && <InfoItem icon={Globe}     label="Website"              value={user.businessInfo.website} isLink />}
            </div>
          </div>
        )}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={user.accountType === 'business' ? 'John Doe (account owner)' : 'Your full name'}
            ...
            />
            {user.accountType === 'business' && (
              <p className="text-xs text-gray-500 mt-1">The person who owns this business account</p>
            )}
          </div>
          ...
          {user.accountType === 'business' && (
            <div>
              ...
              <input
                type="text" name="businessInfo.companyName" value={formData.businessInfo.companyName}
                onChange={handleChange} required placeholder="Acme Corporation"
                className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 dark:text-white"
              />
            </div>
          )}
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              name="address.country" value={formData.address.country} onChange={handleChange}
              className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 dark:text-white"
            >
          ...
          <input
            type="text" name="address.postalCode" value={formData.address.postalCode} onChange={handleChange}
      ...
      {/* Business info — business accounts only */}
      {user.accountType === 'business' && (
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
          <h4 className="text-md font-semibold text-gray-900 dark:text-white mb-1">Business Information</h4>
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">Leave a dropdown on "Keep Current" to preserve the existing value.</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input
              type="text" name="businessInfo.companyName" value={formData.businessInfo.companyName}
            ...
            />
            <select
              name="businessInfo.companyType" value={formData.businessInfo.companyType} onChange={handleChange}
              className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 dark:text-white"
            >
              <option value="">Keep Current ({user.businessInfo?.companyType?.replace(/_/g, ' ') || 'Not set'})</option>
            ...
            <select
              name="businessInfo.industry" value={formData.businessInfo.industry} onChange={handleChange}
              className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 dark:text-white"
            >
              <option value="">Keep Current ({user.businessInfo?.industry?.replace(/_/g, ' ') || 'Not set'})</option>
            ...
            <input
              type="text" name="businessInfo.taxId" value={formData.businessInfo.taxId}
              onChange={handleChange} placeholder="Tax ID"
              className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 dark:text-white"
            />
            <input
              type="text" name="businessInfo.registrationNumber" value={formData.businessInfo.registrationNumber}
              onChange={handleChange} placeholder="Registration Number"
              className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 dark:text-white"
            />
            <input
              type="url" name="businessInfo.website" value={formData.businessInfo.website}
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input
            type="url" name="socialLinks.twitter" value={formData.socialLinks.twitter}
            onChange={handleChange} placeholder="https://twitter.com/yourhandle"
            className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 dark:text-white"
          />
          <input
            type="url" name="socialLinks.linkedin" value={formData.socialLinks.linkedin}
            onChange={handleChange} placeholder="https://linkedin.com/in/yourprofile"
            className="w-full px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-gray-900 dark:text-white"
          />
          <input
            type="url" name="socialLinks.website" value={formData.socialLinks.website}
